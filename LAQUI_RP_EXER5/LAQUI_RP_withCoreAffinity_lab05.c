...
int **create_matrix(int n) {
    ...
    for (int i = 0; i < n; i++) {
         ...
         if (!x[i]) { printf("Memory allocation failed\n"); exit(1);} 
    }
...
int send_all(int sock, void *buf, int len) {
    int total = 0;
    while (total < len) {
        int s = send(sock, (char*)buf + total, len - total, 0);
        if (s <= 0) return -1;
        total += s;
    }
    return 0;
...
int lab05(){
    ...
    while (userInput)
    {
        ...
        if (t <= 0) { printf("No slaves available\n"); exit(1);} 
        ...
        for (int i =0; i<t; i++){
            ...
            if (usable_cores <= 0) usable_cores = 1; 
            int core_id = i % usable_cores;

            if (send_all(socket_desc, &core_id, sizeof(int)) < 0) { perror("Unable to send core_id"); return -1; }
            
            // Send the metadata to server: 
            if(send_all(socket_desc, &rows, sizeof(int)) < 0){  printf("Unable to send message\n"); return -1; } 
            if(send_all(socket_desc, &n, sizeof(int)) < 0){  printf("Unable to send message\n"); return -1; } 
        ...
        if (recv_all(client_sock, &core_recv, sizeof(int)) < 0) { 
            perror("recv"); 
            return -1; 
        }
